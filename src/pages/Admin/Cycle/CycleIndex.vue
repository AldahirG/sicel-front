...
const sortByCycle = (arr) => {
  return [...arr].sort((a, b) => {
    const cycleA = a.cycle?.toUpperCase?.() || '';
    const cycleB = b.cycle?.toUpperCase?.() || '';
...
onMounted(async () => {
  await store.getAll();
  cycles.value = sortByCycle(store.cycles);
});
</script>
    ...
    <Table>
      ...
      <template #content>
        <TableRow v-for="cycle in cycles" :key="cycle.id">
          ...
          <TableDataCell>
            <router-link
              :to="{ path: '/admin/cycles/' + cycle.id + '/edit/' }"
