...
const fetchLeads = async () => {
  try {
    const { data } = await lead.getAll(currentPage.value, { paginated: true, 'per-page': 10000 });
...
const clearFilters = () => {
  ...
  localStorage.removeItem('adminLeadScrollTop');
  localStorage.removeItem('adminLeadPage');
  localStorage.removeItem('adminLeadFilters');
  localStorage.removeItem('adminLeadAppliedFilters');
};
...
const saveLeadAdminViewState = () => {
  const table = document.querySelector('.scrollbar-thin');
  if (table) {
    localStorage.setItem('adminLeadScrollTop', table.scrollTop.toString());
  }
  localStorage.setItem('adminLeadPage', currentPage.value.toString());
  localStorage.setItem('adminLeadFilters', JSON.stringify(filters.value));
  localStorage.setItem('adminLeadAppliedFilters', JSON.stringify(appliedFilters.value));
};

const restoreLeadAdminViewState = () => {
  const savedFilters = localStorage.getItem('adminLeadFilters');
  const savedAppliedFilters = localStorage.getItem('adminLeadAppliedFilters');
  const savedPage = localStorage.getItem('adminLeadPage');
...
const restoreScrollPosition = () => {
  const scrollTop = parseInt(localStorage.getItem('adminLeadScrollTop'), 10);
  const table = document.querySelector('.scrollbar-thin');
...
<template>
  ...
  <div class="overflow-x-auto max-h-[calc(100vh-280px)] border rounded shadow-inner scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
    <Table class="min-w-[1280px]">
      <template #header>
        <TableRow>
          <TableHeaderCell>Check ✓</TableHeaderCell>
          <TableHeaderCell>Promotor Asignado <ColumnFilter field="promoter.name" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Nombre Completo</TableHeaderCell>
          <TableHeaderCell>Género <ColumnFilter field="information.genre" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Seguimiento <ColumnFilter field="information.followUp.name" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Carrera interés <ColumnFilter field="information.careerInterest" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Grado Escolar <ColumnFilter field="grade.name" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Programa <ColumnFilter field="program" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Teléfono</TableHeaderCell>
          <TableHeaderCell>Correo Electrónico</TableHeaderCell>
          <TableHeaderCell>Medio <ColumnFilter field="asetName.contactType" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Asset <ColumnFilter field="asetName.name" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Campaña <ColumnFilter field="campaign.name" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>País <ColumnFilter field="address.country.name" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Estado <ColumnFilter field="address.state" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Ciudad <ColumnFilter field="address.city" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Beca <ColumnFilter field="scholarship" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
          <TableHeaderCell>Interno/Externo <ColumnFilter field="intern" :leads="leads" @filter-applied="handleHeaderFilter" /></TableHeaderCell>
        ...
        >
          ...
          <TableDataCell>
            <div class="relative inline-block">
              <button @click="toggleDropdown(lead.id)" class="bg-gray-200 hover:bg-gray-300 text-black px-3 py-2 rounded-md">
                <i class="bi bi-three-dots"></i>
              </button>
              <div v-if="dropdowns[lead.id]" class="dropdown-menu absolute bg-white shadow-lg rounded-md border border-gray-300 flex flex-col" :style="{ top: '100%', right: '0', minWidth: '160px', zIndex: 1050 }">
                <router-link :to="{ path: '/admin/leads/' + lead.id + '/assignment' }" class="block px-4 py-2 text-white bg-indigo-500 hover:bg-indigo-600 rounded-t-md">
                  <i class="bi bi-person-plus-fill mr-2"></i>Asignar
                </router-link>
                <router-link :to="{ path: '/admin/leads/' + lead.id + '/show' }" class="block px-4 py-2 text-white bg-blue-500 hover:bg-blue-600">
                  <i class="bi bi-eye-fill mr-2"></i>Mostrar
                </router-link>
                <router-link :to="{ path: '/admin/leads/' + lead.id + '/edit' }" class="block px-4 py-2 text-black bg-amber-400 hover:bg-amber-500">
                  <i class="bi bi-pencil-square mr-2"></i>Editar
                </router-link>
                <button @click="confirmDelete(lead.id)" class="block w-full text-center px-4 py-2 text-white bg-red-500 hover:bg-red-600 rounded-b-md">
    ...
    <div class="flex justify-center mt-4">
      <Pagination :current-page="currentPage" :total-pages="totalPages" @page-changed="fetchLeads" />
    </div>
</template>
...
/* Estilo para el menú desplegable de los filtros */
.column-filter-menu {
...
/* Opcional: mejora visual del scroll en el filtro */
.column-filter-menu::-webkit-scrollbar {
  width: 6px;
}
.column-filter-menu::-webkit-scrollbar-thumb {
  background-color: #9ca3af;
  border-radius: 4px;
}
.column-filter-menu::-webkit-scrollbar-track {
